...
**Étape 3 Construire 🟢 (EF-05).** Le **canvas de scénario** est en place (`core/construire.mjs`, API `/v1/ideas/:id/scenarios`) : initialisation depuis la sélection Cartographier (nœuds/ponts reportés), composition, édition et retrait de scénarios typés (rupture/prudente/optimiste) — CRUD immuable dédupliqué, chaque opération horodatée et journalisée. Le moteur valide et aggrège ; le contenu narratif reste fourni par l'utilisateur ou le Synthesizer LLM (EF-06/EF-07 en attente de génération assistée).
