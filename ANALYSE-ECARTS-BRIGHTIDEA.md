...
**Étape 3 Construire 🟢 (EF-05/EF-06).** Le **canvas de scénario** est en place (`core/construire.mjs`, API `/v1/ideas/:id/scenarios`) : initialisation depuis la sélection Cartographier (nœuds/ponts reportés), composition, édition et retrait de scénarios typés (rupture/prudente/optimiste) — CRUD immuable dédupliqué, chaque opération horodatée et journalisée. Le **Collision Mode** est en place (`core/collision.mjs`, API `/v1/ideas/:id/collision`) : paires de concepts **distants** (≥ plancher 60, jamais déjà liés par le réseau ou l'historique), nouveauté déterministe, score nouveauté × faisabilité calculé **seulement si la faisabilité est fournie** (LLM/humain importé, jamais deviné), timeline append-only + sélection des collisions mémorisée et journalisée. Le contenu narratif des scénarios reste fourni par l'utilisateur ou le Synthesizer LLM (EF-07 brief structuré en attente de génération assistée).
