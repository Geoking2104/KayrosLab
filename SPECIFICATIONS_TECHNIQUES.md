...
- **Collision Mode (EF-06/F3-F7 🟢 implémenté).** `core/collision.mjs` — `distanceConcepts` (distance réelle par partage de tags, jaccard), `idCollision` (id stable par paire triée), `normalizeCollision` (2 concepts requis, faisabilité clampée 0–100), `scoreCollision` (**nouveauté × faisabilité / 100, `null` sans faisabilité**), `runCollisionMode` (paires ≥ plancher — défaut 60 —, ignore arêtes du réseau + historique déjà collisionné, tri par score), `addCollision` (timeline append-only horodatée + signée, dédup), `rapportCollision` (comptages réels : total paires, score max/min, rendu clair). La faisabilité/proposition est **importée** (LLM/humain via `scores[]`), jamais devinée.
- **API Collision Mode.** `POST /v1/ideas/:id/collision` (concepts depuis le body ou `construire`/`cartographie.tendances`, plancher optionnel, `scores[]` d'apport, persiste `construire.collisions`, événement `construire.collision`), `GET /v1/ideas/:id/collision` (rapport `rapportCollision`), `POST /v1/ideas/:id/collision/selection` (mémorise les collisions choisies dans `construire.selectionCollisions`, événement `construire.collision.select`).
