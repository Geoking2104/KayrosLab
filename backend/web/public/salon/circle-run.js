...
(function () {
  ...
  function appendBubble(opts) {
    ...
    li.innerHTML = av + "<div><header><strong></strong><span></span><time datetime=\"" + at + "\"></time></header><p></p></div>";
  ...
  function localVoice(author, question) {
    ...
    return (name + " ouvre " + work + ". A votre question — " + q + " — je reponds depuis la page. " + core).slice(0, 520);
  }
  ...
  function runTable(question, ids) {
    ...
    guests.forEach(function (author, i) {
      chain = chain.then(function () {
        ...
        return askVoice(author, question).then(function (text) {
          if (wait) {
            ...
            appendBubble({ authorId: author.id, name: authorNameSafe(author), act: "prend la parole", text: text });
          }
