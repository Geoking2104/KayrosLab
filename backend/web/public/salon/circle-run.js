...
(function () {
  ...
  function hookMentions() {
    ...
    draft.addEventListener("input", function () {
      ...
      var hits = guestIds().map(function (id) {
        var a = findAuthor(id); return { id: id, name: a ? authorNameSafe(a) : id };
      }).filter(function (h) { return h.id.indexOf(needle) === 0 || h.name.toLowerCase().indexOf(needle) >= 0; });
  ...
  function proofLangMismatch(text) {
    ...
    return userLang() === "fr" ? (en >= 3 && en > fr) : (fr >= 3 && fr > en);
  }
  ...
  function proofFromAuthor(author) {
    if (!author) return "";
    var bits = [];
    (author.memory || []).slice(0, 2).forEach(function (m) {
      var quote = String(m.sample || m.text || "").replace(/\s+/g, " ").trim().slice(0, 280);
      if (quote) bits.push("\u00ab " + quote + (quote.length >= 280 ? "\u2026" : "") + " \u00bb" + (m.title ? " \u2014 " + m.title : ""));
    });
    var raw = bits.join(" ");
