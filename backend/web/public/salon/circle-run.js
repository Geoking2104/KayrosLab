...
(function () {
  ...
  function localVoice(author, question, seat) {
    ...
    var open = "Je prends la question — \u00ab " + q + " \u00bb — et je refuse d'y repondre par une maxime. " + (blurb ? ("On me connait ainsi : " + blurb + " ") : "") + "Ce n'est pas un extrait qui parle a votre place : c'est une position.";
    var mid = role === "objecteur"
      ? ("J'objecte a la lecture. Elle arrange trop vite ce qui reste trouble. " + work + " m'apprend a ne pas confondre l'ordre d'un recit et l'ordre d'une vie. Tout expliquer n'est pas encore rendre libre : c'est seulement rendre descriptible.")
      : role === "defenseur"
      ? ("Je tiens la these contre l'objection. L'objection n'est pas nulle ; elle change d'objet. " + work2 + " montre qu'une precision n'est pas une fuite. On peut accorder le malheur sans accorder le systeme qui le baptise.")
      : role === "secretaire"
      ? "Je minute. Une lecture veut voir clair ; une objection refuse que la clarte tienne lieu de justice. Le conflit n'est pas un malentendu de vocabulaire. Il porte sur ce qu'on doit a l'experience une fois qu'on l'a nommee."
      : ("Je lis d'abord. " + work + " ne me donne pas une formule : il me donne une maniere. Partir du cas, ecarter le systeme trop poli, puis conclure. La liberte n'est pas le contraire de l'explication ; c'est ce qui reste quand l'explication a tout dit et que l'action n'a pas commence.");
    var close = "Donc je ne cite pas pour finir. Tant qu'on n'a pas dit qui agit, et a quel prix, la table n'a fait que deplacer les mots. Cultiver un jardin, fonder une loi, ou se taire : trois suites. Laquelle tenez-vous ?";
    return { text: (open + " " + mid + " " + close).replace(/\s+/g, " ").trim(), proof: proofFromAuthor(author) };
  }
  function askVoice(author, question, seat) {
    var system = "Tu es " + authorNameSafe(author) + ". " + localizedBlurb(author) + " Role: " + ((seat && seat.role) || "invite") + ". " + ((seat && seat.brief) || "") + " Ecris 160 a 260 mots, premiere personne, francais. Raisonne (these, consequence, reserve). Interdit: reciter un extrait comme seule reponse. Les livres sont une methode, pas un stock de citations.";
    showThink("Recherche de la voix de " + authorNameSafe(author) + " — construction.");
    ...
    return fetch(DEMO, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ system: system, user: question }), signal: ctrl ? ctrl.signal : undefined })
      .then(function (res) { return res.json().then(function (j) { return { ok: res.ok, j: j }; }); })
      .then(function (r) {
        var text = (r.j && (r.j.text || r.j.content)) || "";
        if (!r.ok || !text || /^\[mock\]/i.test(text) || text.replace(/\s+/g, " ").trim().length < 90) return localVoice(author, question, seat);
  ...
  function hookTalk() {
    ...
    btn.addEventListener("click", function (e) {
      ...
      showThink("Le salon prend la parole. Recherche, puis raisonnement.");
