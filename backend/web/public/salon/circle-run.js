...
(function () {
  ...
  function proofLangMismatch(text) {
    var t = String(text || "");
    var en = (t.match(/\b(the|and|of|to|that|which|with|from|this)\b/gi) || []).length;
    var fr = (t.match(/\b(le|la|les|des|une|que|qui|dans|pour|avec)\b/gi) || []).length;
    var lang = userLang();
    if (lang === "fr") return en >= 3 && en > fr;
    return fr >= 3 && fr > en;
  }
  function fallbackProof(author) {
    var titles = ((author && author.works) || []).slice(0, 2).join(", ");
    var blurb = localizedBlurb(author).replace(/\s+/g, " ").trim().slice(0, 240);
  ...
  function proofFromAuthor(author) {
    ...
    var raw = bits.join(" ");
    if (!raw || proofLangMismatch(raw)) return fallbackProof(author);
    return raw;
  }
  function localizeProof(author, proof) {
    ...
    var lang = userLang();
    logStep(lang === "en" ? "Translating excerpt into English..." : "Traduction de l'extrait...", true);
    var system = lang === "en"
      ? "Translate the cited excerpt into English. Keep book titles. Reply with the translated excerpt only, no preamble."
      : "Traduis l'extrait cite en francais. Conserve les titres. Reponds uniquement par l'extrait traduit, sans preambule.";
  ...
  function localVoice(author, question) {
    ...
    var core = blurb || (userLang() === "en" ? "The table advances by reading, not by summary." : "La table n avance que si l on relit.");
    return { text: (name + " / " + work + " — " + q + " — " + core).slice(0, 520), proof: proofFromAuthor(author) };
  }
  function askVoice(author, question) {
    var lang = userLang();
    var system = "Tu es " + authorNameSafe(author) + (author.era ? " (" + author.era + ")" : "") + ". " + localizedBlurb(author) + " Livres: " + ((author.works || []).slice(0, 5).join(", ") || "domaine public") + (lang === "en" ? ". Answer in English, 70-130 words, first person. No markdown." : ". Reponds en francais, 70 a 130 mots, a la premiere personne. Pas de markdown.");
    ...
    return fetch(DEMO, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ system: system, user: question }) })
      ...
      .then(function (r) {
        var text = (r.j && (r.j.text || r.j.content)) || "";
        if (!r.ok || !text || /^\[mock\]/i.test(text)) { logStep("Mode local pour " + authorNameSafe(author) + "."); return localVoice(author, question); }
        ...
        return askVoice(author, question).then(function (out) {
          ...
          return localizeProof(author, proof).then(function (localized) {
            if (wait) {
              ...
              appendBubble({ authorId: author.id, name: authorNameSafe(author), act: "prend la parole", text: text, proof: localized });
            }
