...
export function parseInbound(text) {
  ...
  if (mention) {
    return { kind: 'address', handle: mention[1], body: raw.replace(mention[0], '').trim() };
  }
...
export function openingProtocol({ circleName, question, seats, locale = 'fr' }) {
  ...
  if (locale === 'en') {
    return [`The table is set. Circle ${circleName}.`, `Question: ${question}`, `Seated: ${list}`, 'Address a guest (@voltaire). Say “let the salon speak” to hear them answer one another.', 'Voices generated from public-domain works. This number is The Salon, not the author.'].join('\n');
  }
  return [`La table est dressée. Cercle ${circleName}.`, `Question : ${question}`, `À table : ${list}`, 'Adressez un convive (@voltaire). Dites « laissez parler » pour les entendre se répondre.', 'Voix générées à partir d’œuvres du domaine public. Ce numéro est Le Salon, pas l’auteur.'].join('\n');
}
